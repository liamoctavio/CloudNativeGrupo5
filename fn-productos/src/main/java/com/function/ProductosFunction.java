...
public class ProductosFunction {
  ...
  public HttpResponseMessage productosRoot(
      @HttpTrigger(name = "req", methods = { HttpMethod.GET,
          HttpMethod.POST }, authLevel = AuthorizationLevel.ANONYMOUS, route = "productos") HttpRequestMessage<Optional<String>> request,
  ...
  public HttpResponseMessage productosById(
      @HttpTrigger(name = "req", methods = { HttpMethod.GET, HttpMethod.PUT,
          HttpMethod.DELETE }, authLevel = AuthorizationLevel.ANONYMOUS, route = "productos/{id}") HttpRequestMessage<Optional<String>> request,
  ...
  private HttpResponseMessage obtener(HttpRequestMessage<?> req, long id) throws SQLException, IOException {
    try (Connection con = Db.connect();
      ...
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next())
          return json(req, map(rs), HttpStatus.OK);
  ...
  try {
    ...
    try (Connection con = Db.connect()) {
      if (in.getBodegaId() == null) {
        Long defId = getDefaultBodegaId(con);
        if (defId == null) {
          return badRequest(req, "{\"error\":\"No hay bodega por defecto configurada (DEFAULT_BODEGA_ID)\"}");
        }
        in.setBodegaId(defId);
      }
  ...
  private HttpResponseMessage actualizar(HttpRequestMessage<Optional<String>> req, long id) {
    try {
      ...
      try (Connection con = Db.connect();
        ...
        data.put("precio", in.getPrecio());
        if (in.getBodegaId() != null)
          data.put("bodegaId", in.getBodegaId());
  ...
  private static Long getDefaultBodegaId(Connection con) throws SQLException {
    ...
    String defCode = System.getenv().getOrDefault("DEFAULT_BODEGA_CODE", "BOD-GENERAL");
    try (PreparedStatement ps = con.prepareStatement("SELECT ID FROM BODEGAS WHERE CODIGO=?")) {
      ps.setString(1, defCode);
      try (ResultSet rs = ps.executeQuery()) {
